...
description: >-
  ...
  corp-doctor / weekly-planning / weekly-retro operating cycle. Triggers on
...
Keep this skill reusable: owner, repositories, paths, board IDs and routing come from the user’s local Manager Config. Never bundle session logs, customer examples, private repository references, or a copy of a personal installation.

## Setup

Before first use, define this in your project's `AGENTS.md` (preferred) or `CLAUDE.md` (compatibility). If the project has no agent config yet, run `corp-doctor` to create or repair it.
...
`corp-doctor` is the bootstrap/repair skill for this config. Title-type metadata is NOT configured here: it lives in labels, the parent tree, and Projects (see Issue title convention).
...
New issues start in the configured backlog lane. The first proposal, draft, specification, or decision counts as work: save the substantive draft in the issue body and move it to the configured active lane.
