...
graph LR
    ...
    P --> D["daily<br/>живая полная неделя"]
    D --> T["задачи<br/>по отделам"]
...
В каждом созданном отделе есть локальный скилл `daily`. Он обновляет живой `WNN-plan.html`: вся ISO-неделя остаётся видимой, прошлые дни хранят факт, сегодня выделено, будущие дни остаются планом, а привычки Wealth сохраняются только в браузере.
