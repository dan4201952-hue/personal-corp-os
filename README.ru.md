...
Правила для агента это обычный текстовый файл `AGENTS.md` в папке. Никакой магии: markdown, который вы читаете глазами и правите руками.

Агент открывает его первым и узнаёт три вещи: кто здесь работает и что кому можно, в каком порядке читать остальные файлы, куда идти за нужным фактом.

Вот как выглядит начало такого файла в шаблоне штаба:

![Файл правил агента: markdown с таблицей ролей и порядком чтения](docs/images/rules.png)

Файл лежит прямо в папке, рядом с остальными:

![Содержимое папки штаба: AGENTS.md, CLAUDE.md, README.md, me.md, tasks.md и папка skills](docs/images/folder.png)

`CLAUDE.md` рядом нужен для того, чтобы те же правила прочитал Claude Code. Внутри у него одна строка со ссылкой на `AGENTS.md`, поэтому правила остаются в одном месте.

## Главная идея: представление задаёт папка

Такие правила лежат в каждой папке, и содержание у них разное.

Запускаю агента из штаба, он читает правила штаба и видит карту всей системы. Запускаю из отдела, читает правила отдела и видит свой домен, свои задачи, свои скиллы. Модель одна, представление разное. Задаёт его папка, а длинный промпт для этого не нужен.
...
Отделы лежат рядом со штабом, на одном уровне с ним:

```text
Documents/GitHub/
├── HQ/                  штаб: карта системы и точка входа
├── контент/             отдел
├── исследования/        отдел
└── продажи/             отдел
```

Устроен отдел так же, как штаб, и добавляет своё:

![Содержимое шаблона отдела: skills, tasks, AGENTS.md, CLAUDE.md, README.md](docs/images/department.png)
...
Скилл это записанный способ работы, который можно повторить. Тоже обычный файл: `SKILL.md` внутри папки `skills/`.

![Файл скилла: заголовок с именем и описанием, потом граница и шаги](docs/images/skill.png)

Сверху служебный блок с именем и описанием, дальше обычный текст: где граница работы, какие шаги, когда остановиться.

Агент не угадывает, какой скилл взять. В правилах папки лежит таблица, которая связывает запрос человека с файлом:

```md
| Запрос человека     | Что читать и исполнять        |
|---------------------|-------------------------------|
| задачи на сегодня   | skills/daily/SKILL.md         |
| ретро, обзор недели | skills/retro/SKILL.md         |
| планирование недели | skills/planning/SKILL.md      |
```

Отсюда и получается разное поведение в разных папках. Слово "ретро" в отделе ведёт к скиллу отдела и собирает срез отдела. То же слово в штабе ведёт к скиллу штаба и собирает срез всей системы.
...
Первый уровень это один файл со списком на сегодня. Выглядит он так:

![Файл tasks.md: разделы Сегодня, На неделе и Правила](docs/images/tasks.png)

Начинайте с него. Дальше переходите, когда предыдущий уровень начал мешать.

| Уровень | Как выглядит | Когда пора |
|---|---|---|
| 1 | Один `tasks.md` с задачами на сегодня | Всегда, это старт |
| 2 | День, длительные задачи и история разложены по файлам | Контекст задач приходится помнить головой |
| 3 | Состояние длительных задач в GitHub Issues, день сводится к списку ссылок | Появились несколько репозиториев или другие люди |
